...
function getNextRoundStatus(){
 ...
 const allChosen=hasPlayers&&chosen===players.length;
 const allReady=allChosen&&players.every(p=>!!p.selectedCardId);
 return {players,chosen,total:players.length,hasPlayers,allChosen,allReady};
}
...
function renderCurrent(){if(!state)return;const total=state.config.numberMax-state.config.numberMin+1;const n=state.currentNumber;const called=(state.calledNumbers||[]).length;const remaining=total-called;$('current-number').textContent=n??'–';$('remaining-label').textContent=remaining;$('called-count').textContent=called;if($('mobile-current-number'))$('mobile-current-number').textContent=n??'–';if($('mobile-called-count'))$('mobile-called-count').textContent=called;if($('mobile-remaining'))$('mobile-remaining').textContent=remaining;}
...
function renderNextRoundControls(){
 ...
 if(!s.hasPlayers){
   ...
   btn.disabled=false;
   btn.textContent='🏠 เปิดรอผู้เล่นชุดใหม่';
   btn.dataset.mode='reset';
 }else if(!s.allChosen){
   const offlineCount=(state.players||[]).filter(p=>!p.isHost&&!p.connected).length;
   statusEl.textContent=`⏳ รอผู้เล่นเลือกว่าจะใช้บัตรเดิม/รับบัตรใหม่ ${s.chosen}/${s.total} คน${offlineCount?` • 🔌 ออฟไลน์ ${offlineCount} คน (ไม่ได้นับ)`:''}`;
   btn.disabled=true;
   ...
   const offlineCount=(state.players||[]).filter(p=>!p.isHost&&!p.connected).length;
   statusEl.textContent=`✅ ผู้เล่นที่ออนไลน์พร้อมแล้ว ${s.total}/${s.total} คน${offlineCount?` • 🔌 ออฟไลน์ ${offlineCount} คน`:''}`;
   btn.disabled=false;
...
function renderResults(ws){const list=$('results-list');list.innerHTML=ws.length?ws.map((w,i)=>`<div class="result-row"><b>${i+1}. ${esc(w.playerName)}</b><span>บัตร #${pad(w.cardNumber)}</span></div>`).join(''):'<div class="empty">ยังไม่มีผู้ชนะ</div>';
 const showChoices=!me.isHost; $('play-again-options').classList.toggle('hidden',!showChoices); $('results-wait-hint').classList.toggle('hidden',showChoices || me.isHost);
