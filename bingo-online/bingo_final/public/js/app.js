...
socket.on('room_state',s=>{state=s;renderAll();});
...
function renderAll(){
 ...
 $('btn-go-pick-card').classList.toggle('hidden',state.gameStatus!=='lobby');$('btn-start-game').classList.toggle('hidden',!(me.isHost&&state.gameStatus==='lobby'));$('host-start-hint').classList.toggle('hidden',!(me.isHost&&state.gameStatus==='lobby'));
 renderLobbyPlayers();renderCurrent();renderCalledHistory();renderNumberBoard();renderWinners(state);renderHostPlayers();
 const mine=state.players.find(p=>p.playerId===me.playerId);$('lobby-my-status').innerHTML=mine?.selectedCardId?`✅ บัตรของคุณ <strong>#${pad(getCardNumber(mine.selectedCardId))}</strong> พร้อมเล่น`:'🎟️ คุณยังไม่ได้เลือกบัตร';
...
function renderLobbyPlayers(){const list=$('lobby-player-list');list.innerHTML='';$('player-online-count').textContent=`${state.players.filter(p=>p.connected).length} ออนไลน์`;state.players.forEach(p=>{const e=document.createElement('div');e.className='player-row';e.innerHTML=`<div class="avatar">${p.isHost?'👑':'🎟️'}</div><div class="player-info"><b>${esc(p.playerName)}</b><small>${p.isHost?'Host':p.selectedCardId?'เลือกบัตรแล้ว':'กำลังเลือกบัตร'}</small></div><span class="online ${p.connected?'on':''}"></span>`;list.appendChild(e);});}
...
$('btn-select-this-card').onclick=()=>{if(!selectedPreviewCard)return;socket.emit('select_card',{cardId:selectedPreviewCard.cardId},res=>{if(!res?.ok)return toast(res.error,'error');$('modal-preview').classList.add('hidden');toast(`เลือกบัตร #${pad(res.card.cardNumber)} แล้ว`,'success');renderCardGrid();});};
...
function renderHostPlayers(){const list=$('host-player-list');if(!list||!state)return;$('host-player-count').textContent=state.players.length;list.innerHTML=state.players.map(p=>`<div class="host-player"><div><b>${p.isHost?'👑 ':''}${esc(p.playerName)}</b><small>${p.selectedCardId?'บัตร #'+pad(getCardNumber(p.selectedCardId)):'ยังไม่เลือกบัตร'} • ✓ ${p.markedCount||0}</small></div><span class="online ${p.connected?'on':''}"></span></div>`).join('');}
